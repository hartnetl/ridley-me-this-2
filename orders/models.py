...
class Order(models.Model):
    ...
    country = CountryField(blank_label='Choose country *', null=False,
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.order_number:
            # If the current order doesn't have an order number, one is
            # assigned
...
# datefield-add-30-days-to-the-provided-value/2199371
...
class Turtles(models.Model):
    product = models.OneToOneField(Product, on_delete=models.CASCADE,
                                   related_name='turtle', null=True)
    sponsored_status = models.BooleanField(default=False, null=False,
                                           blank=False)
    species = models.CharField(choices=SPECIES, max_length=10, null=False,
                               blank=False)
    name = models.CharField(max_length=50)
    sponsorship_start = models.DateField(auto_now_add=True)
    sponsorship_end = models.DateField(default=get_sponsorship_end)
    turtle_id = models.CharField(max_length=10, unique=True, null=True,
                                 blank=True)
